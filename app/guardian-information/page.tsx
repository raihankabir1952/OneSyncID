...
import { Menu, Bell, Mail, Search, Info, Pencil, ChevronDown, ChevronUp, Plus, Users } from "lucide-react";
...
function TextField({ label, placeholder, value, onChange, hasTag = false }: {
  label: string; placeholder: string; value: string; onChange: (v: string) => void; hasTag?: boolean;
}) {
  return (
    <div className="flex flex-col gap-[6px] py-[14px]" style={{ borderBottom: "1px solid #d9d9d9" }}>
  ...
  return (
    <div className="w-full">
      ...
      >
        ...
        {open ? <ChevronUp size={20} className="text-[#5e5757]" /> : <ChevronDown size={20} className="text-[#5e5757]" />}
      </button>
      {open && (
...
export default function GuardianInformationPage() {
  ...
  const updateGuardian = (id: number, key: keyof GuardianEntry, val: string) => {
    setGuardians((prev) => prev.map((g) => g.id === id ? { ...g, [key]: val } : g));
  };
      ...
      <div className="w-full max-w-[393px] bg-white min-h-screen flex flex-col">
        ...
        >
          ...
          {guardians.map((guardian) => (
            <div key={guardian.id} className="flex flex-col w-full" style={{ marginBottom: "16px" }}>
              ...
              <div className="flex flex-col gap-[8px] py-[14px]" style={{ borderBottom: "1px solid #d9d9d9" }}>
                ...
                <div className="relative" style={{ width: "180px" }}>
                  <select
                    ...
                    style={{
                      ...fontSwitzer, fontSize: "14px",
                      color: guardian.relationship ? "#000" : "#5e5757",
                      border: "1px solid #d9d9d9", outline: "none", width: "100%",
                    }}
                  >
              ...
              <div className="flex flex-col gap-[6px] py-[14px]" style={{ borderBottom: "1px solid #d9d9d9" }}>
                ...
                <input
                  ...
                  style={{ ...fontSwitzer, fontSize: "16px", color: guardian.dob ? "#000" : "#a09898", border: "none", outline: "none", background: "transparent", width: "100%" }}
                />
              </div>
              ...
              <SelectField label="BLOOD GROUP" placeholder="Select blood group" options={BLOOD_GROUPS} />
              <SelectField label="RELIGION" placeholder="Select religion" options={RELIGIONS} />
              <SelectField label="MARITAL STATUS" placeholder="Select marital status" options={MARITAL_STATUSES} />
            </div>
          ))}

          {/* Add another guardian */}
          <button
            onClick={() => setGuardians((prev) => [...prev, { id: Date.now(), relationship: "", firstName: "", lastName: "", dob: "", nid: "" }])}
