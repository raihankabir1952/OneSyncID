...
export default function RelativesPage() {
  ...
  const updateRelative = (id: number, key: keyof RelativeEntry, val: string) => {
    setRelatives((prev) => prev.map((r) => r.id === id ? { ...r, [key]: val } : r));
  };
    ...
    <div className="min-h-screen bg-gray-100 flex justify-center">
      <div className="w-full max-w-[393px] bg-white min-h-screen flex flex-col">
        ...
        >
          ...
          {/* Relative's Identity label */}
          <span style={{ ...fontSwitzer, fontSize: "14px", fontWeight: 500, color: "#000", letterSpacing: "0.14px", marginBottom: "8px" }}>
            Relative's Identity
          </span>

          {/* Relative entries */}
          {relatives.map((relative) => (
            <div key={relative.id} className="flex flex-col w-full" style={{ marginBottom: "16px" }}>
              <TextField
              ...
              <div className="flex flex-col gap-[6px] py-[14px]" style={{ borderBottom: "1px solid #d9d9d9" }}>
                ...
                <input
                  type="date"
                  value={relative.dob}
                  onChange={(e) => updateRelative(relative.id, "dob", e.target.value)}
                  style={{ ...fontSwitzer, fontSize: "16px", color: relative.dob ? "#000" : "#a09898", border: "none", outline: "none", background: "transparent", width: "100%" }}
                />
              </div>
          ...
          <button
            onClick={() => setRelatives((prev) => [...prev, { id: Date.now(), firstName: "", lastName: "", dob: "", nid: "" }])}
