...
// ── Data ──────────────────────────────────────────────────────────────────────
const sessions = [
  { id: 1, name: "City Bank",   logo: "/images/city-bank-logo.png", lastUsed: "Last used 2 mins ago",  href: "/sso/city-bank" },
  { id: 2, name: "BUET",        logo: "/images/buet-logo.png",       lastUsed: "Last used 2 days ago",  href: "" },
  { id: 3, name: "Pubali Bank", logo: "/images/pubali-bank-logo.png",lastUsed: "Last used 3 days ago",  href: "" },
];
...
// ── Other components ──────────────────────────────────────────────────────────
function SessionRow({ name, logo, lastUsed, href }: { name: string; logo: string; lastUsed: string; href?: string }) {
  const router = useRouter();
  return (
    <button
      type="button"
      onClick={() => href && router.push(href)}
      className="w-full flex items-center justify-between rounded-[12px] px-[20px] py-[16px] text-left bg-white"
      style={{ border: "1px solid #d9d9d9" }}
    >
      <div className="flex items-center gap-[8px] min-w-0">
        <div className="relative w-[40px] h-[40px] shrink-0 rounded-[4px] overflow-hidden bg-white">
          <Image src={logo} alt={name} fill className="object-contain" />
        </div>
        <div className="flex flex-col gap-[3px]">
          <span className="text-black text-[16px]" style={{ ...fontSwitzer, fontWeight: 500 }}>{name}</span>
          <span className="text-[#5e5757] text-[14px]" style={{ ...fontSwitzer, fontWeight: 400 }}>{lastUsed}</span>
        </div>
      </div>
      <span className="text-[#ff3838] text-[14px] whitespace-nowrap shrink-0" style={{ ...fontSwitzer, fontWeight: 400 }}>Revoke</span>
    </button>
  );
}
      ...
      <div className="w-full max-w-[393px] bg-white min-h-screen flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto flex flex-col gap-[20px] pb-[20px]">
          <div className="flex flex-col gap-[20px] px-[20px] pt-[20px]">
            <span className="text-[#333] text-[16px]" style={{ ...fontSwitzer, fontWeight: 500 }}>SSO Access</span>
            <div className="flex flex-col overflow-hidden rounded-[12px] w-full" style={{ border: "1px solid rgba(2,95,201,0.2)", backgroundColor: "rgba(2,95,201,0.05)" }}>
              <div className="flex items-start gap-[8px] p-[16px]" style={{ borderBottom: "1px solid #d9d9d9" }}>
                <div className="flex items-center justify-center shrink-0 w-[24px] h-[24px] rounded-[8px]" style={{ backgroundColor: "rgba(2,95,201,0.1)" }}>
                  <QrCode size={14} className="text-[#025fc9]" />
                </div>
                <div className="flex flex-col gap-[10px] flex-1">
                  <p className="text-[#333] text-[16px] tracking-[0.16px]" style={{ ...fontSwitzer, fontWeight: 400 }}>Sign in to any connected services</p>
                  <button type="button" className="flex items-center justify-center px-[10px] py-[8px] rounded-[8px] w-[84px]" style={{ backgroundColor: "#025fc9" }}>
                    <span className="text-white text-[16px] tracking-[0.16px] whitespace-nowrap" style={{ ...fontSwitzer, fontWeight: 500 }}>Scan QR</span>
                  </button>
                </div>
              </div>
            </div>
          ...
          <div className="flex flex-col gap-[20px] px-[20px] pb-[10px]">
            <div className="flex items-center justify-between">
              <span className="text-[#333] text-[16px]" style={{ ...fontSwitzer, fontWeight: 500 }}>Active Sessions</span>
              <button type="button"><span className="text-[#025fc9] text-[14px]" style={{ ...fontSwitzer, fontWeight: 400 }}>See All</span></button>
            </div>
            <div className="flex flex-col gap-[10px]">
              {sessions.map((s) => <SessionRow key={s.id} name={s.name} logo={s.logo} lastUsed={s.lastUsed} href={s.href} />)}
            </div>
          </div>
        </div>
        ...
        <div className="sticky bottom-0 z-10 w-full flex items-center justify-between bg-white px-[10px] pt-[8px] pb-[10px]" style={{ borderTop: "1px solid #d9d9d9", boxShadow: "0 -2px 10px rgba(0,0,0,0.04)" }}>
          ...
          <BottomNavItem label="SSO" active icon={<ShieldCheck size={22} strokeWidth={1.8} />} onClick={() => router.push("/sso")} />
